...
const StaffPortal: React.FC<Props> = ({ user, entries, setUser }) => {
  ...
  const monthTips = myEntries
    .filter(e => {
      const d = new Date(e.date);
      return d.getMonth() === currentMonth.getMonth() && d.getFullYear() === currentMonth.getFullYear() && e.type === 'TIP';
    })
    .reduce((acc, curr) => acc + curr.amount, 0);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500 pb-32 px-4 md:px-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        ...
        <div className="bg-amber-500 p-6 rounded-[2.5rem] text-white shadow-xl shadow-amber-100 flex flex-col justify-between">
          <div>
            <div className="bg-white/20 w-8 h-8 rounded-xl flex items-center justify-center mb-3"><Coins className="w-4 h-4" /></div>
            <p className="text-amber-50 text-[9px] font-bold uppercase tracking-widest">Alınan Toplam Bahşiş</p>
          </div>
          <p className="text-2xl font-black mt-2">{monthTips.toLocaleString()} TL</p>
        </div>
