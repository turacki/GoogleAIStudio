...
import { Plus, Trash2, Edit2, Save, X, RefreshCw, AlertTriangle, AlertCircle, ShieldAlert, Download, Upload, Eye, EyeOff, ShieldCheck } from 'lucide-react';
...
const AdminPersonnel: React.FC<Props> = ({ users, setUsers }) => {
  ...
  const validatePassword = (pass?: string) => {
    ...
    // En az 4 karakter, en az 1 rakam, en az 1 harf
      ...
      {isAdding && (
        <div className="bg-white p-10 rounded-[3rem] border-2 border-indigo-100 shadow-sm animate-in zoom-in-95">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Saatlik Ücret</label>
              <input type="number" className="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold border border-transparent focus:border-indigo-500 transition-all outline-none" value={editForm.hourlyRate || 0} onChange={e => setEditForm({ ...editForm, hourlyRate: Number(e.target.value) })} />
            </div>
            <div className="space-y-1">
          ...
          <tbody className="divide-y divide-slate-50">
            {users.map(user => (
              <tr key={user.id} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="px-10 py-6">
                  {editingId === user.id ? (
                    <input type="number" className="bg-white border-2 border-indigo-500 rounded-xl px-4 py-2 text-sm font-bold w-24" value={editForm.hourlyRate} onChange={e => setEditForm({...editForm, hourlyRate: Number(e.target.value)})} />
                  ) : (
